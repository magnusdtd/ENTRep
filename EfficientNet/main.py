...
def parse_args():
    parser = argparse.ArgumentParser(description='EfficientNetB0 Training/Evaluation Script')
...
def main(args):
    ...
    if args.mode == 'hold-out':
        ...
        model.show_learning_curves('./results/EfficientNetB0_hold_out_learning_curve.png')
        ...
        kf.run()
        kf.show_learning_curves('./results/EfficientNetB0_k_fold_learning_curve.png')
        model.load_state_dict(kf.get_best_model_state_dict())

    exp_name = 'EfficientNetB0.pth'
    ...
    make_submission(saved_model, 'EfficientNetB0', device, test_df)
