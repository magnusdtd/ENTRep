...
def parse_args():
    parser = argparse.ArgumentParser(description='DenseNet121 Training/Evaluation Script')
...
def main(args):
    ...
    if args.mode == 'hold-out':
        ...
        model.show_learning_curves('./results/DenseNet121_hold_out_learning_curve.png')
        ...
        kf.run()
        kf.show_learning_curves('./results/DenseNet121_k_fold_learning_curve.png')
        model.load_state_dict(kf.get_best_model_state_dict())

    exp_name = 'DenseNet121.pth'
    model.save_model_state(exp_name)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    saved_model = DenseNet.load_model(
        exp_name, 
    ...
    make_submission(saved_model, 'DenseNet121', device, test_df)
